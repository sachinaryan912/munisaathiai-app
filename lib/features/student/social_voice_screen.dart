...
class SocialVoiceScreen extends StatelessWidget {
  const SocialVoiceScreen({super.key});

  @override
  Widget build(BuildContext context) {
    final repo = StudentRepository();
    return AppShell(
      title: 'Socially Strong',
      showAiFab: false,
      body: AsyncScreen<List<Map<String, dynamic>>>(
        loader: repo.getSocialVoiceLogs,
...
class _BodyState extends State<_Body> {
  ...
  Widget build(BuildContext context) {
    ...
    return Stack(
      children: [
        widget.list.isEmpty
            ? ListView(children: const [SizedBox(height: 120), EmptyView(title: 'No entries yet', subtitle: 'Log a social odd you raised your voice against.', icon: HugeIcons.strokeRoundedMegaphone01)])
            : ListView.builder(
                  ...
                  return Padding(
                    ...
                    child: SectionCard(
                      child: Column(
                        ...
                        children: [
                          Text(e['issue'] as String, style: TextStyle(fontWeight: FontWeight.w800, fontSize: 13.5, color: s.textPrimary)),
                          Text(e['date'] as String, style: TextStyle(fontSize: 10.5, color: s.textMuted)),
                          if ((e['actionTaken'] as String?)?.isNotEmpty ?? false) ...[
                            const SizedBox(height: 6),
                            Text(e['actionTaken'] as String, style: TextStyle(fontSize: 12, color: s.textSecondary)),
                          ],
                        ],
                      ),
                    ),
        ...
        Positioned(right: 16, bottom: 16, child: FloatingActionButton(heroTag: 'log_social_voice', backgroundColor: AppColors.saffron500, onPressed: _openLog, child: const Icon(Icons.add, color: Colors.white))),
      ],
    );
  }
}
